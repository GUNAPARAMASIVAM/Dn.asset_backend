...
exports.ClaudeValuationService = exports.ReferenceRateValuationService = exports.NullValuationService = void 0;
exports.format_inr = format_inr;
exports.load_uploads = load_uploads;
exports.get_valuation_service = get_valuation_service;
exports.provider_status = provider_status;
...
const RATES_PATH = process.env.DN_ASSET_MARKET_RATES || path_1.default.resolve(BASE_DIR, '../../backend/market_rates.json');
...
function round_to(value, nearest) {
    return Math.round(value / nearest) * nearest;
}
function load_rates() {
    try {
        const data = fs_1.default.readFileSync(RATES_PATH, 'utf-8');
...
class RateBook {
    ...
    get source() { return this.data.source || "unspecified"; }
    get last_reviewed() { return this.data.last_reviewed || "unknown"; }
    get adjustments() { return this.data.adjustments || {}; }
    unit_to_sqft(unit) {
        if (!unit)
            return null;
        return this.data.unit_to_sqft?.[unit] || null;
    }
    base_range(city, property_type) {
...
function area_in_sqft(payload, book) {
    let value;
    let unit;
...
function missing_fields(payload, book) {
    const missing = [];
    ...
    if (!payload.property_age)
        missing.push("Property age");
    if (!payload.condition)
        missing.push("Current property condition");
...
function confidence(payload, book) {
    let score = 0;
    if (payload.built_up_area || payload.land_area)
        score += 2;
    else if (payload.property_size)
        score += 1;
    if (payload.property_age)
        score += 1;
    if (payload.condition)
        score += 1;
    if (payload.locality || payload.pincode)
        score += 1;
    if (payload.location_features && payload.location_features.length > 0)
        score += 1;
    return score >= 2 ? "Medium" : "Low";
}
function describe_factors(payload) {
    const factors = [
    ...
    if (payload.property_age)
        factors.push("Property age considered");
    if (payload.condition)
        factors.push("Property condition considered");
    if (payload.amenities?.length || payload.location_features?.length)
        factors.push("Amenities and location features considered");
    if (payload.images?.length || payload.documents?.length)
        factors.push("Uploaded details considered");
    return factors;
}
class NullValuationService {
    name = "none";
...
exports.NullValuationService = NullValuationService;
class ReferenceRateValuationService {
    name = "reference";
    book;
    constructor(book) {
    ...
    async estimate(payload) {
        ...
        if (!payload.locality && !payload.pincode)
            spread += parseFloat(adj.spread_no_locality || "0.06");
        if (!payload.property_age)
            spread += parseFloat(adj.spread_missing_details || "0.04");
        if (bucket && !exact_area)
            spread += bucket.spread_bonus;
...
exports.ReferenceRateValuationService = ReferenceRateValuationService;
...
class ClaudeValuationService {
    ...
    _get_client() {
        ...
        this._client = new sdk_1.default({ apiKey: process.env.ANTHROPIC_API_KEY });
        return this._client;
    }
    _property_brief(payload) {
        const area = area_in_sqft(payload, this.book);
        const base = this.book.base_range(payload.location, payload.property_type);
        const lines = [
            "PROPERTY DETAILS",
    ...
    _encode(ref, block_type) {
        if (!ref.path || !fs_1.default.existsSync(ref.path)) {
            logger.warn(`Upload ${ref.file_id} missing on disk - skipped`);
            return null;
        }
        const size = fs_1.default.statSync(ref.path).size;
        if (size > MAX_BYTES_TO_AI) {
            logger.info(`Upload ${ref.file_id} is ${size} bytes - too large for the AI call`);
            return null;
        }
        const data = fs_1.default.readFileSync(ref.path).toString("base64");
        return {
            type: block_type,
    ...
    async _to_result(data, payload, response) {
...
exports.ClaudeValuationService = ClaudeValuationService;
const _service_cache = {};
function get_valuation_service() {
    ...
    if (!_service_cache[key]) {
        if (key === "reference")
            _service_cache[key] = new ReferenceRateValuationService();
        else if (key === "claude")
            _service_cache[key] = new ClaudeValuationService();
        else
            _service_cache[key] = new NullValuationService();
    }
